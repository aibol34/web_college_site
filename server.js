...
// new addition for schedule_page
const multer = require('multer');
const csv = require('csv-parser');
const mammoth = require('mammoth');
const AdmZip = require('adm-zip');
...
// In-memory cache (unused now but retained if needed later)
let cache = {
  html: null,
  timestamp: null
};
...
// ===== Inject Color Styling into HTML Tables =====
function injectColorsIntoHTMLTables(html, tableColorMaps) {
  const tableBlocks = html.match(/<table[\s\S]*?<\/table>/gi);
  if (!tableBlocks) return html;
...
app.post('/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    // ===== ✅ Write final HTML to cache file =====
    fs.writeFileSync(CACHE_FILE, finalHTML, 'utf-8');
...
// ===== Configuration =====
// const SECRET_PASSKEY = 's3cr3t42'; // Ideally from env variable
const SECRET_PASSKEY = '00aa00aa';
const CACHE_LIFETIME_MS = 24 * 60 * 60 * 1000; // 24 hours
...
// ===== Inject Color Styling into HTML =====
function injectColorsIntoHTMLTables(html, tableColorMaps) {
  const tableBlocks = html.match(/<table[\s\S]*?<\/table>/gi);
  if (!tableBlocks) return html;
  ...
  // Replace old tables with updated ones
  let resultHTML = html;
  tableBlocks.forEach((original, i) => {
    resultHTML = resultHTML.replace(original, updatedTables[i]);
  });
...
// ===== Cached Data Access Endpoint =====
app.get('/cached-data', (req, res) => {
  const now = Date.now();
  if (cache.html && cache.timestamp && (now - cache.timestamp) < CACHE_LIFETIME_MS) {
    res.send(cache.html);
  } else {
    res.status(404).send('No cached data.');
  }
